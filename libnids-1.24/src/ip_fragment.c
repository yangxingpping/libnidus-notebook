...
ip_defrag(struct ip *iph, struct sk_buff *skb)
{
	...
	if (!hostfrag_find(iph) && skb)  //根据目标ip查找在【fragtable】中是否已经存在分片
		hostfrag_create(iph);	//直接创建(前提是skb不为空)	
	...
	if (this_host)	//在没有Ip分片的情况下，【this_host】为空
		qp = ip_find(iph);
	else
		qp = 0;
	...
	offset &= IP_OFFSET;
	if (((flags & IP_MF) == 0) && (offset == 0)) {	//说明没有分片
		if (qp != NULL)
	...
	if (!this_host) //在内存不够，调用了 ip_evictor()之后重新分配
		hostfrag_create(iph);

	offset <<= 3;			/* offset is in 8-byte chunks */
	ihl = iph->ip_hl * 4;   //iph->ip_h1表示ip头的长度，单位是(32bit,4字节)
	...
	*/
	if (qp != NULL) {
		...
		if (offset == 0) {		//ip分片的首个分片到达
			qp->ihlen = ihl;
			memcpy(qp->iph, iph, ihl + 8);
		}
		del_timer(&qp->timer);	//删除关于这个分片的定时器删除掉，
		qp->timer.expires = jiffies() + IP_FRAG_TIME;	/* about 30 seconds */
		qp->timer.data = (unsigned long)qp;	/* pointer to queue */
		qp->timer.function = ip_expire;	/* expire function */
		add_timer(&qp->timer); //重新建立这个分片的超时定时器
	}
	else {
	...
	end = offset + ntohs(iph->ip_len) - ihl;  //end=当前分片的有效数据的尾偏移

	/* Point into the IP datagram 'data' part. */
	ptr = (unsigned char *)(skb->data + ihl);  //ptr指向当前数据包的有效数据的首地址
	...
	*/
	...
	eliminated.
	*/
	if (prev != NULL && offset < prev->end) {  //发生了重叠
		nids_params.syslog(NIDS_WARN_IP, NIDS_WARN_IP_OVERLAP, iph, 0);
		i = prev->end - offset;   //i=当前分片和上一个分片重叠的字节数
		offset += i;		/* ptr into datagram */  //把当前分片的有小数据向后移动i个字节
	...
	*/
	for (tmp = next; tmp != NULL; tmp = tfp) {
		...
		tmp->len -= i;		/* so reduce size of    */  //减小原来缓存链表节点的长度
		...
		*/
		if (tmp->len <= 0) { //缓存链表中的节点长度调整后如果小于0，则删除
			if (tmp->prev != NULL)
	...
	*/
	...
	layer...
	*/
	if (ip_done(qp)) { //检查是否一个连续的ip分片是否收到
...
ip_defrag_stub(struct ip *iph, struct ip **defrag)
{
  ...
  numpack++;
  timenow = 0;
  while (timer_head && timer_head->expires < jiffies()) {  //如果缓存的头节点超时，直接调用它的清理函数
  ...
  if ((*defrag = (struct ip *)ip_defrag((struct ip *) (skb->data), skb)))
	return IPF_NEW;		//一个ip分片的完整的数据包到达

  return IPF_ISF;	//一个Ip分片
}
